// @MX:NOTE: 디자인 상품 등록 페이지 (SPEC-SKIN-006)
...
const DesignCreatePage = () => {
  ...
  const handleSave = async (values) => {
    setSaving(true);
    try {
      await productApi.create({ ...values, type: 'design' });
      setToast('디자인 상품이 등록되었습니다.');
  ...
  return (
    <div className="min-w-[1280px] p-6 space-y-4">
      <h1 className="text-xl font-bold text-[--huni-fg-default]">디자인 상품 등록</h1>
      <GeneralProductForm productType="design" onSave={handleSave} onCancel={() => window.history.back()} loading={saving} />
      {toast && (
