// @MX:NOTE: 포장재 등록 페이지 (SPEC-SKIN-006)
...
const PackagingCreatePage = () => {
  ...
  const handleSave = async (values) => {
    setSaving(true);
    try {
      await productApi.create({ ...values, type: 'packaging' });
      setToast('포장재가 등록되었습니다.');
  ...
  return (
    <div className="min-w-[1280px] p-6 space-y-4">
      <h1 className="text-xl font-bold text-[--huni-fg-default]">포장재 등록</h1>
      <GeneralProductForm productType="packaging" onSave={handleSave} onCancel={() => window.history.back()} loading={saving} />
      {toast && (
...
export default PackagingCreatePage;
