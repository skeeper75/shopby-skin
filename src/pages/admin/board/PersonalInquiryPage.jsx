...
import { getPersonalInquiries, replyToPersonalInquiry } from '../../../services/admin/board';
...
const PersonalInquiryPage = () => {
  ...
  const fetchData = useCallback((params) => getPersonalInquiries(params), [refreshKey]);
  ...
  const handleReplySubmit = async (item, content) => {
    setIsReplying(true);
    try {
      await replyToPersonalInquiry(item.id, content);
      setIsPanelOpen(false);
      setRefreshKey((k) => k + 1);
    } catch (err) {
      console.error('[PersonalInquiryPage] 답변 실패:', err);
    } finally {
      setIsReplying(false);
    }
  };
